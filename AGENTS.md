...
- Deck content is JSON-backed (`data/`) — no CMS
- Session storage uses Supabase Postgres via Drizzle ORM (`lib/schema.ts`, `lib/db.ts`)
- Auth is Supabase anonymous + OAuth — do not replace or bypass it
...
- Audio files are static MP3s in `public/audio/` generated by `npm run generate:audio`
